...
from common.test_utils.common.utils import (
    replace_calendar_ids_in_contract,
    replace_workflow_ids_in_contract,
...
def upload_contracts(contracts: Dict[str, Dict[str, Any]]) -> None:
    ...
    for product_id, contract_properties in contracts.items():
        ...
        contractdata = replace_workflow_ids_in_contract(
            contractdata, endtoend.testhandle.workflow_definition_id_mapping
        )

        contractdata = replace_schedule_tag_ids_in_contract(
            contractdata, endtoend.testhandle.schedule_tag_ids_to_e2e_ids
        )

        contractdata = replace_calendar_ids_in_contract(
            contractdata, endtoend.testhandle.calendar_ids_to_e2e_ids
        )
...
def create_account(
    ...
    if account_id:
        post_body["account"].update(id=account_id)
