...
from common.test_utils.common.utils import replace_calendar_ids_in_contract
...
class PerformanceTest(endtoend.AcceleratedEnd2EndTest):
    ...
    def load_contracts(cls) -> None:
        ...
        for contract_name, path in cls.contracts.items():
            ...
            cls.contract_contents[contract_name] = replace_calendar_ids_in_contract(
                contractdata=str(load_file_contents(path)),
                id_mapping={},
            )
    ...
    def setup_performance_test_profile(
        ...
        for dg in self.test_profile["dataloader_setup"]["dependency_groups"]:
            ...
            if id_base < 100 or id_base > 999:
                log.warning(
                    f"Unexpected id_base: {id_base}, should be between 100 - 999"
                )
    ...
    def run_performance_test(
        ...
        account_ids = self.setup_performance_test_profile(test_type)
        ...
        if test_type == PerformanceTestType.SCHEDULES:
            ...
            self._postings_performance_test(account_ids)
        else:
    ...
    def _schedules_performance_test(self, account_ids: List[str]) -> None:
        account_schedules = get_account_schedules(account_ids[0])
        schedule_groups = get_schedule_groups(account_schedules)
        first = True
        for tag, details in self.paused_tags.items():
            schedule_next_run_times = get_schedule_tag_next_run_times(account_ids[0])
            for schedule_group in schedule_groups.values():
                skip_previous_schedules_in_schedule_group(
                    tag, schedule_group, schedule_next_run_times
                )
    ...
    def _configure_postings_stages(self):
        ...
        if "stage_range" in self.test_profile["postings_setup"]:
            ...
            # Update the profile with the calculated stages
            self.test_profile["postings_setup"]["stages"] = [
                {
                    "tps": tps,
                    "duration": duration,
                    "timeout": timeout,
    ...
    def _postings_performance_test(self, account_ids: List[str]) -> None:
...
class AccountsPostingsProducer:
    """
    Waits for accounts to be activated and handles the loading of postings for each activated
    account using the postings template.
    """
        ...
        with ThreadPoolExecutor(max_workers=6) as executor:
            ...
            for future in as_completed(future_to_thread_name.keys()):
                ...
                try:
                    ...
                    log.warning(f"Thread {thread_name} raised exception: {exc}")
                else:
    ...
    def _account_activations_consumer(self) -> str:
        ...
        while self.accounts_waiting_activation:
            ...
            if msg.error():
                log.warning(
                    f"account activations consumer got error: {msg.error().str()}"
                )
                continue
            try:
                ...
                log.warning(
                    f"account activations consumer failed to decode message: {e}."
                )
                continue
            ...
            if account_update_wrapper:
                ...
                if account_update["account_id"] in self.accounts_waiting_activation:
                    if account_update["status"] == "ACCOUNT_UPDATE_STATUS_COMPLETED":
                        ...
                        if self.postings_template:
                            self.accounts_to_process.append(
                                account_update["account_id"]
                            )
                        else:
                            self.results.accounts_loaded.append(
                                account_update["account_id"]
                            )
    ...
    def _postings_responses_consumer(self) -> str:
        ...
        while (
            ...
            if msg.error():
                log.warning(
                    f"postings responses consumer got error: {msg.error().str()}"
                )
                continue
            try:
                event_msg = json.loads(msg.value().decode())
                ...
                log.warning(
                    f"postings responses consumer failed to load a posting: {e}."
                )
                continue
    ...
    def _balance_updates_consumer(self) -> str:
        ...
        while (
            ...
            if not msg:
                continue
            if msg.error():
                log.warning(f"balance updates consumer got error: {msg.error().str()}")
                continue
            try:
                event_msg = json.loads(msg.value().decode())
            except Exception as e:
        ...
        for pib_skip in simulation_setup.get("postings_to_skip", {}):
            ...
            for pib in result["result"]["posting_instruction_batches"]:
                ...
                if client_batch_id is None:
                    log.warning(f"PIB is missing a client batch ID: {pib}")
                    continue
                value_timestamp = pib.get("value_timestamp")
                if value_timestamp is None:
                    log.warning(f"PIB is missing a value timestamp: {pib}")
                    continue
                else:
