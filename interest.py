...
def construct_accrual_details(
    ...
    return AccrualDetails(
        ...
        requires_gl_posting=requires_gl_posting,
            ...
            if is_capitalised_accrual:
                ...
                accrual_postings.extend(
                    _create_postings_for_accruals(
                        ...
                        accruals=accruals,
                        tside=account_tside,
                        requires_gl_posting=accrual_detail.requires_gl_posting,
    ...
    for interest_dimension in interest_dimensions:
        ...
        for address in [
            ...
            if address:
                ...
                description = (
                    interest_dimension.instruction_description
                    or "Reversing accrued interest"
                )
                instruction_details = {
    ...
    if accrual_details.balance != Decimal("0"):
        ...
        for tier_name, tier in accrual_details.rates.items():
            ...
            if tier_max < tier_min:
                ...
                tier_balance = min(effective_balance, tier_max) - min(
                    effective_balance, tier_min
                )
...
def _create_postings_for_accruals(
    ...
    if net_postings:
        ...
        instruction_details = _get_posting_instruction_details(
            posting_infos=accruals, event_type=event_type
        )
        ...
        for accrual in accruals:
            instruction_details = _get_posting_instruction_details(
                posting_infos=[accrual], event_type=event_type
            )
            if instruction_description:
                instruction_details["description"] = instruction_description
...
def _create_accrual_postings(
    ...
    if accrual_amount == 0:
        ...
        if reverse:
            debit_internal_account = payable_receivable_mapping.payable_internal_account
            credit_internal_account = payable_receivable_mapping.paid_internal_account
            debit_customer_address = address
            credit_customer_address = "INTERNAL_CONTRA"
        else:
            debit_internal_account = payable_receivable_mapping.paid_internal_account
            credit_internal_account = (
                payable_receivable_mapping.payable_internal_account
            )
            debit_customer_address = "INTERNAL_CONTRA"
            credit_customer_address = address

    else:
        address = payable_receivable_mapping.receivable_address

        if reverse:
            debit_internal_account = (
                payable_receivable_mapping.received_internal_account
            )
            credit_internal_account = (
                payable_receivable_mapping.receivable_internal_account
            )
            debit_customer_address = "INTERNAL_CONTRA"
            credit_customer_address = address
        else:
    ...
    cti_suffix = (
        f"{vault.get_hook_execution_id()}_{address}" f"_{DEFAULT_ASSET}_{denomination}"
    )

    cti_prefix = (
        f"REVERSE_ACCRUED_{charge_type}" if reverse else f"ACCRUE_{charge_type}"
    )
    ...
    accrual_postings = vault.make_internal_transfer_instructions(
        ...
        from_account_address=debit_customer_address,
        to_account_id=vault.account_id,
        to_account_address=credit_customer_address,
        asset=DEFAULT_ASSET,
        instruction_details=instruction_details,
    ...
    # GL postings
    if requires_gl_posting:
        accrual_postings.extend(
            vault.make_internal_transfer_instructions(
                amount=accrual_amount,
                denomination=denomination,
                client_transaction_id=f"{cti_prefix}_GL_{cti_suffix}",
                from_account_id=debit_internal_account,
                from_account_address=DEFAULT_ADDRESS,
                to_account_id=credit_internal_account,
    ...
    for balance in interest_balances:
        if balance is not None:
            ...
            remainder = PostingInfo(
                ...
                description=f"Zero out remainder after accrued "
                f"{charge_type.lower()} applied.",
            )
...
def _create_application_postings(
    ...
    if application_amount == 0:
        ...
        debit_internal_account = payable_receivable_mapping.payable_internal_account
        debit_internal_account_address = DEFAULT_ADDRESS

        credit_internal_account = vault.account_id
        credit_internal_account_address = apply_address

        debit_customer_address = payable_receivable_mapping.payable_address
        credit_customer_address = "INTERNAL_CONTRA"

        cti_suffix = f"{vault.get_hook_execution_id()}_{debit_customer_address}"
        cti_suffix += f"_{DEFAULT_ASSET}_{denomination}"
    else:
        debit_internal_account = vault.account_id
        debit_internal_account_address = apply_address

        credit_internal_account = payable_receivable_mapping.receivable_internal_account
        credit_internal_account_address = DEFAULT_ADDRESS

        debit_customer_address = "INTERNAL_CONTRA"
        credit_customer_address = payable_receivable_mapping.receivable_address
        cti_suffix = f"{vault.get_hook_execution_id()}_{credit_customer_address}"
        cti_suffix += f"_{DEFAULT_ASSET}_{denomination}"
    ...
    application_postings = vault.make_internal_transfer_instructions(
        ...
        denomination=denomination,
        client_transaction_id=f"APPLY_{charge_type}_CUSTOMER_{cti_suffix}",
        from_account_id=vault.account_id,
        from_account_address=debit_customer_address,
        to_account_id=vault.account_id,
        to_account_address=credit_customer_address,
    ...
    application_postings.extend(
        vault.make_internal_transfer_instructions(
            amount=application_amount,
            denomination=denomination,
            client_transaction_id=f"APPLY_{charge_type}_GL_{cti_suffix}",
            from_account_id=debit_internal_account,
            from_account_address=debit_internal_account_address,
            to_account_id=credit_internal_account,
            to_account_address=credit_internal_account_address,
            asset=DEFAULT_ASSET,
            instruction_details=instruction_details,
            override_all_restrictions=True,
        )
