...
description = (
    "A series of common functions that are frequently used by multiple smart contracts"
)
...
def get_parameter(
    ...
    if is_json and parameter is not None:
        ...
        if upper_case_dict_values:
            ...
            parameter = {
                key: [str(i).upper() for i in value] for key, value in parameter.items()
            }
...
def yearly_to_daily_rate(
    ...
    """
    days_in_year = (
        days_in_year if days_in_year in VALID_DAYS_IN_YEAR else DEFAULT_DAYS_IN_YEAR
    )
...
def are_optional_parameters_set(vault, parameters: List[str]) -> bool:
    ...
    """
    return all(
        get_parameter(vault, parameter, optional=True) is not None
...
def get_balance_sum(
    ...
    return Decimal(
        sum(
            balances[(address, DEFAULT_ASSET, denomination, phase)].net
